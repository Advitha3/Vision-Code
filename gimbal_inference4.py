...
class Ultralytics_Inference():
    ...
    def detect(self):
        ...
        smoothed_x2 = None
        smoothed_y2 = None
        POSITION_SMOOTHING = 0.4  # Smoothing factor for position (0 to 1)
        SIZE_SMOOTHING = 0.1     # Smoothing factor for size (lower = slower size change)
                ...
                for det in tracker_detection:
                    ...
                    if self.selected_track_id is not None:
                        ...
                        if int(track_id) == self.selected_track_id:
                            ...
                            if smoothed_x1 is None and smoothed_y1 is None and smoothed_x2 is None and smoothed_y2 is None:
                                smoothed_x1 = x1
                                smoothed_y1 = y1
                                smoothed_x2 = x2
                                smoothed_y2 = y2
                            else:
                                # Calculate smoothed center from previous smoothed box
                                smoothed_cx = (smoothed_x1 + smoothed_x2) / 2
                                smoothed_cy = (smoothed_y1 + smoothed_y2) / 2
                                smoothed_width = smoothed_x2 - smoothed_x1
                                smoothed_height = smoothed_y2 - smoothed_y1

                                
                                # Smooth the center position
                                smoothed_cx = smoothed_cx + POSITION_SMOOTHING * (cx - smoothed_cx)
                                smoothed_cy = smoothed_cy + POSITION_SMOOTHING * (cy - smoothed_cy)
                                print(smoothed_cx,smoothed_cy)
                                
                                # Smooth the size (width and height) with slower adjustment
                                smoothed_width = smoothed_width + SIZE_SMOOTHING * (curr_width - smoothed_width)
                                smoothed_height = smoothed_height + SIZE_SMOOTHING * (curr_height - smoothed_height)
                                
                                # Reconstruct smoothed bounding box coordinates
                                smoothed_x1 = smoothed_cx - smoothed_width / 2
                                smoothed_y1 = smoothed_cy - smoothed_height / 2
                                smoothed_x2 = smoothed_cx + smoothed_width / 2
                                smoothed_y2 = smoothed_cy + smoothed_height / 2
                                self.send_fov_points((smoothed_cx,smoothed_cy))
                            
                
                            plot_one_box([smoothed_x1, smoothed_y1, smoothed_x2, smoothed_y2], frame, label=label, color=(0,0,255), line_thickness=2)
...
#                         plot_one_box([smoothed_x1, smoothed_y1, smoothed_x2, smoothed_y2], frame, label=label, color=RED, line_thickness=2)
